...
from tcvectordb.model.index import Index, FilterIndex, SparseIndex, SparseVector

vdb_url = "http://10.x.x.x"
vdb_key = "xB2i************************************"
db_name = "python-sdk-test-fulltext-search"
coll_name = "sdk_collection_fulltext_search"
...
res = vdb_client.upsert(
    ...
    documents=[
        ...
        {'id': '0005', 'text': texts[4], 'sparse_vector': sparse_vectors[4]},
    ],
    build_index=True,
)
...
# search with text
res = vdb_client.fulltext_search(
    database_name=db_name,
    collection_name=coll_name,
    data=bm25.encode_queries('什么是腾讯云向量数据库'),
